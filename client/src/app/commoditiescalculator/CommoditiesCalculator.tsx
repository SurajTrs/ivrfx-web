...
type Market = "Commodities";

type AccountType = "Retail" | "Professional";

const INSTRUMENTS: Array<{ label: string; symbol: string; contract: number }> = [
  { label: "WTI Crude Oil", symbol: "CL=F", contract: 1 },
  { label: "Brent Crude", symbol: "BZ=F", contract: 1 },
  { label: "Gold (XAU/USD)", symbol: "XAUUSD=X", contract: 1 },
  { label: "Silver (XAG/USD)", symbol: "XAGUSD=X", contract: 1 },
  { label: "Natural Gas", symbol: "NG=F", contract: 1 },
];

export default function CommoditiesCalculator() {
  ...
  return (
    <section className="bg-white">
      <div className="container py-4 py-md-5">
        <div className="alert alert-light border rounded-4 mb-4 calc-info animate-fade-in">
          <div className="d-flex align-items-start gap-3">
            ...
            <div>
              <div className="title text-dark">What is a Commodities Profit Calculator?</div>
              <p className="mb-1 desc">Estimate position value, margin requirement, spread and potential P/L on commodities like Oil, Gold and more.</p>
              <p className="mb-0 desc">Enter your trade parameters to see the breakdown including leverage and overnight swaps.</p>
            </div>
          </div>
        </div>
          ...
          <div className="row g-4">
            <div className="col-12 col-lg-7">
              <div className="row g-3">
                ...
                <div className="col-12">
                  <small className="text-secondary">Positions quoted in a currency different from your account currency may be subject to a conversion fee at exit.</small>
                </div>
              </div>
            </div>
      ...
      <style jsx>{`
        .calc-info { background: #ffffff; border-color: #ecebf1; box-shadow: 0 4px 16px rgba(17, 24, 39, 0.06); transition: transform 240ms ease, box-shadow 240ms ease; position: relative; }
        .calc-info::before { content: ''; position: absolute; left: 0; top: 0; bottom: 0; width: 4px; border-top-left-radius: .75rem; border-bottom-left-radius: .75rem; background: linear-gradient(180deg, #7c3aed 0%, #6d28d9 100%); }
        .calc-info:hover { transform: translateY(-1px); box-shadow: 0 8px 22px rgba(17, 24, 39, 0.08); }
        .calc-info .icon-wrap { display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; border-radius: 50%; background: #f5f3ff; color: #6d28d9; flex: 0 0 36px; border: 1px solid #e9d5ff; }
        ...
        @media (min-width: 768px) { .calc-info .title { font-size: 1.25rem; } .calc-info .desc { font-size: 1.025rem; } }
