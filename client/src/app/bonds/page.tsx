...
export default function BondsAllPage() {
  const [q, setQ] = useState("");
  const [sort, setSort] = useState<"symbol" | "name" | "change">("symbol");
  const [dir, setDir] = useState<1 | -1>(1);

  const rows = useMemo(() => {
    const items = popularBonds.slice();
  ...
  return (
    <main className="container py-5">
      <header className="d-flex flex-wrap align-items-end justify-content-between mb-4">
        <div>
          <h1 className="h3 text-white m-0">All Bonds</h1>
          <div className="text-secondary small">Search and filter popular bonds</div>
        </div>
