...
export default function IndicesAllPage() {
  const [q, setQ] = useState("");
  const [sort, setSort] = useState<"ticker" | "name" | "change">("ticker");
  const [dir, setDir] = useState<1 | -1>(1);

  const rows = useMemo(() => {
    const items = popularIndices.slice();
    const term = q.trim().toLowerCase();
    const filtered = term
      ? items.filter(r => r.ticker.toLowerCase().includes(term) || r.name.toLowerCase().includes(term))
      : items;
    filtered.sort((a, b) => {
  ...
  return (
    <main className="container py-5">
      <header className="d-flex flex-wrap align-items-end justify-content-between mb-4">
        <div>
          <h1 className="h3 text-white m-0">All Indices</h1>
          <div className="text-secondary small">Search and filter major global indices</div>
        </div>
